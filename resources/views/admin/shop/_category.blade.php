...
<div @if($depth > 0) class="ms-4" @endif>
    <div class="card shadow mb-4">
        <div class="card-header d-flex flex-wrap align-items-center justify-content-between gap-2">
            <div>
                ...
                <div class="small text-body-secondary">{{ trans('extended-translation::shop.categories.type') }}</div>
            </div>
            <div class="d-flex flex-wrap align-items-center gap-2">
                ...
                <span class="text-nowrap">
                    @if($categoryTarget)
                        <a href="{{ route('extended-translation.admin.shop.categories.edit', [$category, $categoryTarget]) }}" class="mx-1" title="{{ trans('extended-translation::admin.actions.translate') }}" data-bs-toggle="tooltip">
        ...
        <div class="card-body">
            @if($category->packages->isEmpty())
                <p class="mb-0">{{ trans('extended-translation::shop.packages.empty_category') }}</p>
            @else
                <div class="table-responsive">
                    <table class="table table-striped mb-0">
                        <caption class="visually-hidden">{{ trans('extended-translation::shop.packages.type') }}</caption>
                        <thead>
                        <tr>
                            <th scope="col">#</th>
    ...
    @if($category->relationLoaded('categories'))
        @foreach($category->categories as $child)
            @include('extended-translation::admin.shop._category', ['category' => $child, 'depth' => $depth + 1])
        @endforeach
    @endif
</div>

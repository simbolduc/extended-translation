        ...
        @foreach($locales as $code => $name)
            <li class="nav-item">
                <a class="nav-link @if($code === $locale) active @endif"
                   href="{{ route('extended-translation.admin.shop.offers.edit', [$offer, $code]) }}">
                    {{ $name }}
    ...
    <div class="row g-4">
        <div class="col-lg-5">
            <div class="card shadow mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <span>{{ trans('extended-translation::shop.offers.source') }}</span>
                    <a href="{{ route('shop.admin.offers.edit', $offer) }}" class="btn btn-sm btn-outline-secondary">
                        <i class="bi bi-pencil-square"></i> {{ trans('extended-translation::admin.edit_original') }}
                    </a>
                </div>
                <div class="card-body">
                    <h2 class="h5">{{ $offer->name }}</h2>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('extended-translation.admin.shop.offers.update', [$offer, $locale]) }}" method="POST">
                        @method('PUT')
                        @csrf

                        <div class="mb-3">
                            <label class="form-label" for="nameInput">{{ trans('messages.fields.name') }}</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="nameInput" name="name" value="{{ old('name', $translation->name ?? $offer->name) }}" required maxlength="50">
                        ...
                        @if($translation)
                            <a href="{{ route('extended-translation.admin.shop.offers.destroy', [$offer, $locale]) }}" class="btn btn-danger" data-confirm="delete">

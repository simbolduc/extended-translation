...
@section('content')
        ...
        @foreach($locales as $code => $name)
            <li class="nav-item">
                <a class="nav-link @if($code === $locale) active @endif"
                   href="{{ route('extended-translation.admin.shop.categories.edit', [$category, $code]) }}">
                    {{ $name }}
    ...
    @if($translation?->isStale($category))
    ...
    <div class="row g-4">
        <div class="col-lg-5">
            <div class="card shadow mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <span>{{ trans('extended-translation::shop.categories.source') }}</span>
                    <a href="{{ route('shop.admin.categories.edit', $category) }}" class="btn btn-sm btn-outline-secondary">
        ...
        <div class="col-lg-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('extended-translation.admin.shop.categories.update', [$category, $locale]) }}" method="POST">
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="nameInput" name="name" value="{{ old('name', $translation->name ?? $category->name) }}" required maxlength="50">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control html-editor @error('description') is-invalid @enderror" id="descriptionArea" name="description" rows="5">{{ old('description', $translation->description ?? $category->description) }}</textarea>
                        ...
                        @if($translation)
                            <a href="{{ route('extended-translation.admin.shop.categories.destroy', [$category, $locale]) }}" class="btn btn-danger" data-confirm="delete">

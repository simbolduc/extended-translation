...
@section('content')
    <p class="mb-3">
        <a href="{{ route('extended-translation.admin.wiki.index') }}" class="btn btn-outline-secondary">
        ...
        @foreach($locales as $code => $name)
            <li class="nav-item">
                <a class="nav-link @if($code === $locale) active @endif"
                   href="{{ route('extended-translation.admin.wiki.pages.edit', [$page, $code]) }}">
                    {{ $name }}
    ...
    <div class="row g-4">
        <div class="col-lg-5">
            <div class="card shadow mb-4">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <span>{{ trans('extended-translation::wiki.pages.source') }}</span>
                    <a href="{{ route('wiki.admin.pages.edit', $page) }}" class="btn btn-sm btn-outline-secondary">
                        <i class="bi bi-pencil-square"></i> {{ trans('extended-translation::admin.edit_original') }}
                    </a>
                </div>
                <div class="card-body">
                    <h2 class="h5">{{ $page->title }}</h2>
                    <p class="text-body-secondary">{{ $page->category->name }}</p>
        ...
        <div class="col-lg-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('extended-translation.admin.wiki.pages.update', [$page, $locale]) }}" method="POST">
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="titleInput" name="title" value="{{ old('title', $translation->title ?? $page->title) }}" required maxlength="100">
                        ...
                        @if($translation)
                            <a href="{{ route('extended-translation.admin.wiki.pages.destroy', [$page, $locale]) }}" class="btn btn-danger" data-confirm="delete">
